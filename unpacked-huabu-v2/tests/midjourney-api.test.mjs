...
test("renders an image history switcher that updates the current image", () => {
  ...
  assert.match(bundle, /children: m\.jsx\(nC, \{ size: 15 \}\)/);
  assert.doesNotMatch(bundle, /className: "bare-image-caption nodrag nopan"/);
...
test("styles image history as a downward thumbnail popover", () => {
  ...
  assert.match(css, /\.compact-top \.compact-history-popover[\s\S]*?top:\s*calc\(100% \+ 8px\)/);
});
...
test("lets Agent normalize secondary-menu parameter aliases", () => {
  const agent = fs.readFileSync(
    new URL("../dist/assets/ai2-agent-panel.js", import.meta.url),
    "utf8",
  );
  assert.match(agent, /aspectRatio/);
  assert.match(agent, /resolution/);
  assert.match(agent, /比例/);
  assert.match(agent, /清晰度/);
  assert.match(agent, /data\.ratio/);
  assert.match(agent, /data\.quality/);
  assert.match(agent, /多步骤/);
  assert.match(agent, /全部动作/);
});

test("Agent waits for every requested node run and resolves named run targets", () => {
  const agent = fs.readFileSync(
    new URL("../dist/assets/ai2-agent-panel.js", import.meta.url),
    "utf8",
  );
  assert.match(agent, /async function execActions/);
  assert.match(agent, /await runNodeWithRetry\(newId, b, results\)/);
  assert.match(agent, /resolveNodeId\(a\.id \|\| a\.node \|\| a\.target\)/);
  assert.match(agent, /await runNodeWithRetry\(id, b, results\)/);
  assert.match(agent, /runResult\.ok === false/);
  assert.match(agent, /重试第 2 次/);
  assert.match(agent, /最多自动重试 1 次/);
  assert.match(agent, /runNodeWithRetry/);
});

test("canvas run actions expose failure status to Agent", () => {
  const bundle = fs.readFileSync(
    new URL("../dist/assets/index-v1514-gemini-strict-sizefix-v6.js", import.meta.url),
    "utf8",
  );
  assert.match(bundle, /return \{ ok: !0, id: O \}/);
  assert.match(bundle, /return \{ ok: !1, id: O, error: le \}/);
});

test("pastes clipboard images into new image nodes without breaking node paste", () => {
  const bundle = fs.readFileSync(
    new URL("../dist/assets/index-v1514-gemini-strict-sizefix-v6.js", import.meta.url),
    "utf8",
  );
  assert.match(bundle, /clipboardData,[\s\S]*?U && U\.items/);
  assert.match(bundle, /getAsFile\(\)/);
  assert.match(bundle, /粘贴图片/);
  assert.match(bundle, /onPaste: [A-Za-z_$][\w$]*/);
  assert.doesNotMatch(bundle, /W && B\.key\.toLowerCase\(\) === "v" && \(B\.preventDefault\(\), pe\(\)\)/);
});

test("uses adaptive ratio as the default for image nodes", () => {
  const bundle = fs.readFileSync(
    new URL("../dist/assets/index-v1514-gemini-strict-sizefix-v6.js", import.meta.url),
    "utf8",
  );
  assert.match(bundle, /ratio: "自适应"/);
  assert.match(bundle, /t\.ratio \|\| "自适应"/);
  assert.match(bundle, /x = t\.ratio \|\| "自适应"/);
});

test("Agent shows a collapsed light processing record after completion", () => {
  const agent = fs.readFileSync(
    new URL("../dist/assets/ai2-agent-panel.js", import.meta.url),
    "utf8",
  );
  ...
  assert.match(agent, /思考过程/);
  assert.match(agent, /details/);
  assert.match(agent, /taskLog/);
  assert.match(css, /ai2-agent-thinking/);
  assert.match(css, /color:\s*#a89a88/);
});
...
test("cache-busts the updated canvas assets", () => {
  ...
  assert.match(html, /index-v1514-gemini-strict-sizefix-v6\.js\?v=ui-cleanup-v15-adaptive-ratio/);
  assert.match(html, /ai2-compact-node-patch\.css\?v=ui-cleanup-v21-image-history-top/);
  ...
  assert.match(html, /ai2-agent-loop\.js\?v=agent-loop-v7-plan-repair/);
  assert.match(html, /ai2-agent-panel\.js\?v=agent-v12-plan-repair/);
  assert.match(html, /ai2-agent-panel\.css\?v=agent-v12-plan-repair/);
  assert.match(html, /ai2-rh-runtime-patch\.js\?v=rh-patch-v7/);
});
