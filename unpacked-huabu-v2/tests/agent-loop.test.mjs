...
test("validateDecision normalizes runNode targetAlias to the canonical target reference", () => {
  const task = createEmptyTask("先生成狗再加猫");
  registerPlan(task, DOG_CAT_PLAN);
  task.aliases.dog = "n_dog";
  task.aliases.dogCat = "n_cat";
  const nodes = [
    { id: "n_dog", type: "generateNode", data: { status: "生成成功", image: "dog" } },
    { id: "n_cat", type: "generateNode", data: {} },
  ];
  const r = validateDecision(
    { decisionId: "d_target_alias", type: "action", reason: "运行下游", action: { name: "runNode", targetAlias: { alias: "dogCat" } } },
    makeCtx(task, nodes, [{ id: "e1", source: "n_dog", target: "n_cat" }]),
  );
  assert.equal(r.ok, true, r.error || "targetAlias 应被解析为 target");
});
...
test("invalid taskPlan repair explicitly requires criteria aliases to match declared steps", async () => {
  const bridge = makeFakeBridge();
  const model = makeScriptedModel([
    decision({
      decisionId: "bad_plan",
      ...
      taskPlan: {
        goal: "生成狗",
        steps: [{ stepId: "s1", alias: "source", intent: "生成狗", dependsOn: [] }],
        successCriteria: [{ alias: "dog" }],
      },
      action: { name: "createNode", alias: "source", nodeType: "generateNode", data: { prompt: "狗" } },
    }),
    decision({ decisionId: "clarify_after_repair", type: "clarify", reason: "修复计划前暂停", question: "请确认目标节点。" }),
  ]);
  const result = await runAgentTask({ bridge, callModel: model.call, task: createEmptyTask("生成狗"), deps: makeDeps() });
  assert.equal(result.status, "clarify");
  const repair = model.received[1].at(-1).content;
  assert.match(repair, /successCriteria/);
  assert.match(repair, /逐字复制/);
