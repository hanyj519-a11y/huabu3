...
function rHBase(e) {
  return Ps(e).replace(/\/openapi\/v2$/i, "");
}
async function rHPost(e, t, n, r = 6e5) {
  const o = await co(
    e,
    {
      method: "POST",
  ...
  try {
    ...
    throw new Error(`RunningHub 返回无法解析：${i.slice(0, 300)}`);
  }
  return s;
}
async function rHFetchBlob(e) {
  if (uo(e)) return await (await fetch(e)).blob();
  if (ay(e)) {
    const t = await co($i(e), { method: "GET" }, 12e4);
    if (!t.ok) throw new Error(`参考图下载失败：HTTP ${t.status}`);
    return await t.blob();
  }
  throw new Error("参考图格式不支持，请使用图片节点或可访问的图片 URL。");
}
async function rHUploadImage(e, t, n) {
  const r = await rHFetchBlob(n),
    o = String(r.type || "image/png").includes("jpeg")
      ? "jpg"
      : String(r.type || "").includes("webp")
        ? "webp"
        : String(r.type || "").includes("gif")
          ? "gif"
          : "png",
    i = new FormData();
  i.append("file", r, `ref-${Date.now()}.${o}`);
  const l = await (
    await co(
      `${rHBase(e)}/openapi/v2/media/upload/binary`,
      { method: "POST", headers: { Authorization: `Bearer ${t || ""}` }, body: i },
      3e5,
    )
  ).text();
  let a;
  try {
    ...
    throw new Error(`RunningHub 上传返回无法解析：${l.slice(0, 200)}`);
  }
  const d =
    (a &&
      a.data &&
      (a.data.download_url || a.data.downloadUrl || a.data.fileUrl || a.data.url)) ||
    (a && (a.download_url || a.url));
  if (!d)
    throw new Error(
      (a && (a.errorMessage || a.msg)) || "RunningHub 上传失败：未返回文件地址。",
    );
  return d;
}
async function rHRunWorkflow({ settings: e, nodeData: t, upstreamImages: n, onRequest: r, addLog: o }) {
  const i = (e && e.runningHub) || tn.runningHub;
  if (!i.baseUrl)
    throw new Error("请先在「API 设置 → RunningHub 工作流」填写 API 地址。");
  if (!i.apiKey)
    throw new Error("请先在「API 设置 → RunningHub 工作流」填写 API Key。");
  const rW = (i.workflows || []).find((c) => c.id === t.rhWorkflowRef);
  const s = rW
    ? String(rW.workflowId || "").trim()
    : String(t.workflowId || "").trim();
  if (!s)
    throw new Error(
      (i.workflows || []).length
        ? "请先在节点里选择要运行的 RunningHub 工作流。"
        : "请先在「API 设置 → RunningHub 工作流」添加工作流并填写工作流 ID。",
    );
  const l = rHBase(i.baseUrl);
  const a = rW
    ? Array.isArray(rW.params)
      ? rW.params
      : []
  ...
  let u = 0;
  for (const c of a) {
    const f = String(c.nodeId || "").trim(),
      p = String(c.fieldName || "").trim();
    if (!f || !p) continue;
    if (c.type === "image") {
      const y = (n || [])[u];
      u += 1;
      if (!y) continue;
      r == null || r(`上传参考图 ${u} → ${p}`);
      const x = await rHUploadImage(l, i.apiKey, y);
      d.push({ nodeId: f, fieldName: p, fieldValue: x });
    } else {
      const y = String(c.value == null ? "" : c.value).trim();
      if (!y) continue;
      d.push({ nodeId: f, fieldName: p, fieldValue: y });
    }
  }
  const g = `${l}/openapi/v2/run/workflow/${s}`;
  r == null || r(g);
  const w = await rHPost(
    g,
    { addMetadata: !0, nodeInfoList: d, usePersonalQueue: "false" },
    i.apiKey,
  );
  if (!w.taskId)
    throw new Error(
      w.errorMessage ||
        w.msg ||
        `RunningHub 任务提交失败：${JSON.stringify(w).slice(0, 200)}`,
    );
  o == null || o(`RunningHub 任务已提交：${w.taskId}`, "info");
  const b = Date.now() + 18e5;
  await new Promise((x) => window.setTimeout(x, 3e3));
  while (Date.now() < b) {
    const x = await rHPost(`${l}/openapi/v2/query`, { taskId: w.taskId }, i.apiKey, 6e4);
    const _ = String((x && x.status) || "").toUpperCase();
    if (_ === "SUCCESS") {
      const N = ((x && x.results) || [])
        .filter((M) => M && M.url)
        .filter((M) =>
          /^(png|jpe?g|webp|gif|bmp|image)$/i.test(
            String(M.outputType || "").replace(/^\./, ""),
          ),
        )
        .map((M) => M.url);
      if (!N.length)
        ((x && x.results) || []).forEach((M) => {
          M && M.url && N.push(M.url);
        });
      if (!N.length) throw new Error("任务成功但没有解析到输出文件地址。");
      return N;
    }
    if (_ === "FAILED")
      throw new Error(
        (x && x.errorMessage) ||
          (x && x.failedReason
            ? `任务失败：${JSON.stringify(x.failedReason).slice(0, 300)}`
            : "RunningHub 任务失败"),
      );
    await new Promise((M) => window.setTimeout(M, 3e3));
  }
  throw new Error("RunningHub 任务超时（30 分钟），可稍后在任务与账单页查看结果。");
}
...
function Vp(e, t = {}, n = "generateNode") {
  const r = Kl(e, t.platformId);
  Dt(e, r);
  return { platformId: r, model: Si(e, { ...t, platformId: r }, n) };
}
function Bp(e = {}, t = {}) {
...
function KC(e = [], t = []) {
  const n = new Set(["generateNode", "videoNode", "runningHubNode", "textNode"]),
...
async function runTextChat({ platform: e, model: t, prompt: n, onRequest: r }) {
  const o = Xt(e, "/chat/completions");
  r == null || r(o);
  const i = await wi(
    o,
    { model: t, stream: !1, messages: [{ role: "user", content: n }] },
    e.apiKey,
    6e5,
  );
  const s =
    i && i.choices && i.choices[0]
      ? (i.choices[0].message && i.choices[0].message.content) || i.choices[0].text
      : null;
  const l = typeof s == "string" ? s.trim() : xi(s).trim();
  if (!l) throw new Error("接口已返回，但没有解析到文本内容。");
  return l;
}
function zs(e) {
  const t = String(e || "").toLowerCase();
...
async function runVideoGeneration({
  ...
  if (!(i != null && i.baseUrl))
    throw new Error(`请先在右上角「API 设置」给「${yf(i)}」填写 API 地址。`);
  if (!(i != null && i.apiKey))
...
`)
  ...
  const c = Xt(i, "/videos/generations");
  o == null || o(c);
  const f = await co(
    c,
    {
    ...
    children: [
      m.jsxs("div", {
        ...
        children: [
          ...
          a ? m.jsx("span", { className: "node-header-right", children: a }) : null,
...
function lM({ id: e, data: t, selected: n }) {
  const [xp, setXp] = z.useState(!1),
    [modOpen, setModOpen] = z.useState(!1),
    C = z.useRef(null),
    s = t.settings || tn,
    l = Kl(s, t.platformId),
    a = Dt(s, l),
    u = Up(a),
    d = (() => {
      const f = String(t.model || "");
      return u.includes(f) ? f : u[0] || "";
    })(),
    hasOut = !!(t.text || "").trim(),
    c = (f) => {
      const p = Dt(s, f),
        A = Up(p),
        x = String(t.model || "");
      t.updateNode?.(e, { platformId: f, model: A.includes(x) ? x : A[0] || "" });
    };
  z.useEffect(() => {
    if (!xp) return;
    const f = (p) => {
      const x = p.target;
      const rhHost = x instanceof Element ? x.closest(".react-flow__node") : null;
      (!rhHost || rhHost.getAttribute("data-id") !== e) && setXp(!1);
    };
    return (
  ...
  z.useEffect(() => {
    ...
    const f = (p) => {
      const x = p.target;
      x instanceof Element && !x.closest(".compact-panel") && setModOpen(!1);
    };
    return (
      document.addEventListener("pointerdown", f),
      () => document.removeEventListener("pointerdown", f)
  ...
  return m.jsxs(Ai, {
    ...
    status: t.status,
    selected: n,
    className: "text-node compact-node",
    children: [
      m.jsx(lt, { type: "target", position: q.Left, className: "handle target" }),
      m.jsx("button", {
        type: "button",
        className: "compact-stage text-node-stage",
        title: xp ? "收起编辑面板" : "展开编辑面板",
        onPointerDown: (f) => {
          C.current = { x: f.clientX, y: f.clientY, moved: !1 };
        },
        onPointerMove: (f) => {
          C.current &&
            f.buttons === 1 &&
            Math.hypot(f.clientX - C.current.x, f.clientY - C.current.y) > 4 &&
            (C.current.moved = !0);
        },
        onClick: (f) => {
          const x = C.current;
          if (x && (x.moved || Math.hypot(f.clientX - x.x, f.clientY - x.y) > 4)) {
            return;
          }
          setXp(!xp);
        },
        children: [
          hasOut
            ? m.jsx("span", { className: "text-node-out nodrag nopan", children: t.text })
            : m.jsx("span", {
                className: "compact-placeholder compact-placeholder-textonly",
                children: m.jsx("small", {
                  children: t.prompt ? "已填写指令，点击运行生成" : "点击编辑 / 生成文本",
                }),
              }),
          t.running ? m.jsx("span", { className: "compact-running", children: m.jsx(ao, { size: 14, className: "spin" }) }) : null,
        ],
      }),
      xp
        ? m.jsxs("div", {
            className: "compact-panel nodrag nopan nowheel",
            onPointerDown: Lt,
            onMouseDown: Lt,
            onDoubleClick: Lt,
            children: [
              (t.upstreamImages || []).length
                ? m.jsx("div", {
                    className: "preview-row gen-up-preview",
                    children: (t.upstreamImages || []).map((f, p) =>
                      m.jsxs(
                        "div",
                        {
                          ...
                          children: [
                            ...
                            m.jsx("button", {
                              ...
                              title: "断开该连接",
                              onClick: () => t.removeUpstreamConnection?.(e, f),
                              children: "×",
                            }),
                          ],
                        },
                ...
                : null,
              m.jsx("textarea", {
                className: "compact-textarea nodrag nopan nowheel",
                value: t.prompt || "",
                placeholder: "输入要生成的文本内容或指令",
                autoFocus: !0,
                onPointerDown: Lt,
                onMouseDown: Lt,
                onChange: (f) => t.updateNode?.(e, { prompt: f.target.value }),
              }),
              m.jsxs("div", {
                ...
                children: [
                  ...
                  m.jsxs("button", {
                type: "button",
                className: `compact-pill relative${modOpen ? " active" : ""}`,
                title: "模型",
                onClick: () => setModOpen(!modOpen),
                children: [
                  m.jsx(vi, { size: 14 }),
                  m.jsx("span", { className: "select-label", children: "模型" }),
                  m.jsx("span", { className: "select-val", children: d || "请选择" }),
                  m.jsx("span", { className: "select-arrow", children: "▾" }),
                  modOpen
                    ? m.jsx(Rr, {
                        type: "genlist",
                        options: u,
                        value: d,
                        onPick: (f) => t.updateNode?.(e, { model: f, _modelPinned: !0 }),
                        onClose: () => setModOpen(!1),
                      })
                    : null,
                ],
              }),
                  m.jsx("button", {
                    type: "button",
                    className: "compact-run",
                    title: "生成文本",
                    onClick: () => t.runNode?.(e),
                    disabled: t.running,
                    children: t.running ? m.jsx(ao, { size: 15, className: "spin" }) : m.jsx(go, { size: 15 }),
                  }),
                ],
              }),
            ],
...
function wSel({ label: e, options: t, value: n, onPick: r, open: o, onToggle: i }) {
  const c = typeof n == "object" ? n.label : n;
  return m.jsxs(m.Fragment, {
    children: [
      m.jsxs("button", {
        type: "button",
        className: `compact-pill relative${o ? " active" : ""}`,
        onClick: i,
        children: [
          m.jsx("span", { className: "select-label", children: e }),
          m.jsx("span", { className: "select-val", children: c || "请选择" }),
          m.jsx("span", { className: "select-arrow", children: "▾" }),
          o ? m.jsx(Rr, { type: "genlist", options: t, value: n, onPick: r, onClose: i }) : null,
        ],
      }),
    ],
  });
...
function wf({ settings: e, value: t, onChange: n }) {
  const [wOpen, setWOpen] = z.useState(!1);
  var i, s;
  const r =
  ...
  z.useEffect(() => {
    ...
    const f = (p) => {
      const x = p.target;
      x instanceof Element && !x.closest(".wf-pop") && setWOpen(!1);
    };
    return (
      document.addEventListener("pointerdown", f),
      () => document.removeEventListener("pointerdown", f)
  ...
  return m.jsx("span", {
    ...
    children: m.jsxs(m.Fragment, {
      children: [
        m.jsxs("button", {
          type: "button",
          className: `compact-pill${wOpen ? " active" : ""}`,
          onPointerDown: Lt,
          onMouseDown: Lt,
          onClick: (f) => {
            f.stopPropagation();
          ...
          children: [
            ...
            m.jsx("span", { children: cur ? yf(cur) : "请选择" }),
            m.jsx("span", { className: "select-arrow", children: "▾" }),
          ],
        }),
        wOpen
          ? m.jsx(Rr, {
              type: "genlist",
              options: r.map((x) => ({ value: x.id, label: yf(x) })),
              value: o,
              onPick: (f) => {
                n == null || n(f);
              },
...
function aM({ id: e, data: t, selected: n }) {
  ...
  return m.jsxs(Ai, {
    ...
    children: [
      ...
      m.jsxs("button", {
        type: "button",
        className: "gen-expand-entry nodrag nopan",
        title: "点击展开编辑面板",
        onPointerDown: Lt,
        onMouseDown: Lt,
        onClick: (f) => {
        ...
        children: [m.jsx(cf, { size: 12 }), " 展开编辑"],
      }),
      m.jsxs("div", {
      ...
      xp
        ? m.jsx("div", {
            className: "gen-expand-overlay nodrag nopan",
            onMouseDown: (f) => {
              if (f.target === f.currentTarget) setXp(!1);
            },
            children: m.jsxs("div", {
              className: "gen-expand-panel nodrag nopan nowheel",
              onPointerDown: Lt,
              onMouseDown: Lt,
              onDoubleClick: Lt,
              children: [
                m.jsxs("div", {
                  className: "gen-expand-head",
                  children: [
                    m.jsxs("span", {
                      className: "gen-expand-title",
                      children: [m.jsx(fl, { size: 15 }), " ", t.title || "图片生成"],
                    }),
                    m.jsx("button", {
                      type: "button",
                      className: "gen-expand-close nodrag nopan",
                      title: "收起",
                      onClick: () => setXp(!1),
                      children: "\u00d7",
                    }),
                ...
                m.jsx("textarea", {
                  className: "gen-expand-textarea nodrag nopan nowheel",
                  value: t.prompt || "",
                  placeholder: "描述你想要生成的画面内容，输入 @ 可引用上游图片",
                  autoFocus: !0,
                  onPointerDown: Lt,
                  onMouseDown: Lt,
                  onChange: (f) => {
                ...
                mt
                  ? m.jsxs("div", {
                      className: "mention-panel in-expand nodrag nopan",
                      children: [
                        m.jsx("div", { className: "mention-title", children: "引用上游图片" }),
                        (t.upstreamImages || []).length
                          ? (t.upstreamImages || []).map((f, p) =>
                              m.jsxs(
                                "button",
                                {
                                  ...
                                  onMouseDown: (y) => {
                                    ...
                                    return (x = t.updateNode) == null
                                      ? void 0
                                      : x.call(t, e, { prompt: (t.prompt || "").replace(/@$/, `图${p + 1}`) });
                                  },
                                  children: [
                                    m.jsx("img", { src: f, alt: "", draggable: !1 }),
                                    m.jsx("span", { children: `图 ${p + 1}` }),
                          ...
                          : m.jsx("div", { className: "mention-empty", children: "还没有上游图片，先从图片节点连线过来" }),
                      ],
                    })
                  : null,
                m.jsxs("div", {
                  className: "gen-expand-toolbar",
                  children: [
                    m.jsx("div", {
                      className: "gen-expand-platform",
                      children: m.jsx(wf, { settings: s, value: l, onChange: c }),
                    }),
                    m.jsxs("button", {
                      type: "button",
                      className: "gen-tool-pill" + (r === "model" ? " active" : ""),
                      onClick: () => i("model"),
                      children: [m.jsx(vi, { size: 13 }), " ", u || "选择模型"],
                    }),
                    m.jsxs("button", {
                      type: "button",
                      className: "gen-tool-pill" + (r === "ratio" ? " active" : ""),
                      onClick: () => i("ratio"),
                      children: [m.jsx(ey, { size: 13 }), " ", t.ratio || "自适应"],
                    }),
                    m.jsxs("button", {
                      type: "button",
                      className: "gen-tool-pill" + (r === "quality" ? " active" : ""),
                      onClick: () => i("quality"),
                      children: [m.jsx(cf, { size: 13 }), " ", t.quality || "1k"],
                    }),
                    m.jsxs("button", {
                      type: "button",
                      className: "gen-tool-pill" + (r === "count" ? " active" : ""),
                      onClick: () => i("count"),
                      children: [m.jsx(eC, { size: 13 }), " ", `${t.count || 1}张`],
                    }),
                    m.jsxs("button", {
                      type: "button",
                      className: "gen-tool-run",
                      onClick: () => {
                        var f;
                        return (f = t.runNode) == null ? void 0 : f.call(t, e);
                      },
                      disabled: t.running,
                      children: [
                        t.running ? m.jsx(ao, { size: 14, className: "spin" }) : m.jsx(go, { size: 14 }),
                        " 生成",
                      ],
                    }),
                  ],
                ...
                m.jsxs("div", {
                  className: "gen-expand-popovers",
                  children: [
                    r === "model"
                      ? m.jsx(Rr, { type: "model", options: d, value: u, onPick: (f) => { var p; return (p = t.updateNode) == null ? void 0 : p.call(t, e, { model: f, _modelPinned: !0 }); }, onClose: () => o(null) })
                      : null,
                    r === "ratio"
                      ? m.jsx(Rr, { type: "ratio", options: [...pC, "自适应"], value: t.ratio || "自适应", onPick: (f) => { var p; return (p = t.updateNode) == null ? void 0 : p.call(t, e, { ratio: f }); }, onClose: () => o(null) })
                      : null,
                    r === "count"
                      ? m.jsx(Rr, { type: "count", options: hC.map((f) => `${f}张`), value: `${t.count || 1}张`, onPick: (f) => { var p; return (p = t.updateNode) == null ? void 0 : p.call(t, e, { count: Number(String(f).replace("张", "")) }); }, onClose: () => o(null) })
                      : null,
                    r === "quality"
                      ? m.jsx(Rr, { type: "quality", options: ["1k", "2k", "3k", "4k"], value: t.quality || "1k", onPick: (f) => { var p; return (p = t.updateNode) == null ? void 0 : p.call(t, e, { quality: f }); }, onClose: () => o(null) })
                      : null,
                  ],
                }),
              ],
  ...
  return m.jsxs(Ai, {
    ...
    children: [
      ...
      m.jsxs("button", {
        ...
        children: [m.jsx(go, { size: 12 }), " 展开编辑"],
      }),
      m.jsxs("div", {
        className: "preview-row",
        children: [
...
m.jsxs("div", {
                  ...
                  children: [
                    m.jsxs("span", {
                      className: "gen-expand-title",
                      children: [m.jsx(go, { size: 15 }), " \u89c6\u9891\u751f\u6210"],
                    }),
                    m.jsx("button", {
                      type: "button",
                      className: "gen-expand-close nodrag nopan",
...
m.jsxs("div", {
                  className: "gen-expand-toolbar",
                  children: [
                    m.jsx("div", { className: "gen-expand-platform", children: m.jsx(wf, { settings: r, value: o, onChange: (s) => i("platformId", s) }) }),
                    m.jsxs("div", { className: "gen-tool-pill static", children: ["\u5c3a\u5bf8 ", m.jsx("select", { value: t.size || "1280x720", onPointerDown: Lt, onMouseDown: Lt, onChange: (s) => i("size", s.target.value), children: ["1280x720", "720x1280", "1920x1080", "1080x1920"].map((s) => m.jsx("option", { value: s, children: s }, s)) })] }),
                    m.jsxs("div", { className: "gen-tool-pill static", children: ["\u65f6\u957f ", m.jsx("select", { value: String(t.duration || "10"), onPointerDown: Lt, onMouseDown: Lt, onChange: (s) => i("duration", s.target.value), children: ["10"].map((s) => m.jsx("option", { value: s, children: s + "s" }, s)) })] }),
                    m.jsxs("button", { type: "button", className: "gen-tool-run", onClick: () => { var C; return (C = t.runNode) == null ? void 0 : C.call(t, e); }, disabled: t.running, children: [t.running ? m.jsx(ao, { size: 14, className: "spin" }) : m.jsx(go, { size: 14 }), " \u751f\u6210\u89c6\u9891"] }),
                  ],
                }),
m.jsx("div", {
                  className: "gen-expand-urls nodrag nopan",
                  children: [
                    m.jsxs("label", { children: ["\u53c2\u8003\u56fe URL\uff08\u6700\u591a 7 \u5f20\uff0c\u6bcf\u884c/\u9017\u53f7\u5206\u9694\uff09", m.jsx("textarea", { className: "settings-model-input", value: t.imageUrls || "", placeholder: "https://example.com/image.png", onPointerDown: Lt, onMouseDown: Lt, onChange: (s) => i("imageUrls", s.target.value) })] }),
                    m.jsxs("label", { children: ["\u53c2\u8003\u89c6\u9891 URL", m.jsx("input", { value: t.videoUrl || "", placeholder: "https://example.com/video.mp4", onPointerDown: Lt, onMouseDown: Lt, onChange: (s) => i("videoUrl", s.target.value) })] }),
                  ],
                }),
              ],
            }),
...
function rhM({ id: e, data: t, selected: n }) {
  const [xp, setXp] = z.useState(!1),
    [rhOpen, setRhOpen] = z.useState(!1),
    C = z.useRef(null),
    v = t.outputImages || [],
    h = v[0],
    rhList = (((t.settings || tn).runningHub || {}).workflows || []),
    rhCur = rhList.find((f) => f.id === (t.rhWorkflowRef || ""));
  z.useEffect(() => {
    if (!xp) return;
    const f = (p) => {
      const x = p.target;
  ...
  return m.jsxs(Ai, {
    title: t.title || "RunningHub 工作流",
    icon: oC,
    status: t.status,
    selected: n,
    className: "runninghub-node compact-node",
    children: [
      m.jsx(lt, { type: "target", position: q.Left, className: "handle target" }),
      m.jsx("button", {
        type: "button",
        ...
        children: [
          h
            ? m.jsx("img", { src: h, alt: "运行结果", draggable: !1 })
            : m.jsxs("span", {
                className: "compact-placeholder",
                children: [m.jsx(oC, { size: 24 }), m.jsx("small", { children: "RunningHub" })],
              }),
          t.running ? m.jsx("span", { className: "compact-running", children: m.jsx(ao, { size: 14, className: "spin" }) }) : null,
        ],
      }),
      xp
        ? m.jsxs("div", {
            className: "compact-top nodrag nopan",
            children: [
              m.jsx("button", {
                type: "button",
                ...
                onClick: (f) => {
                  Lt(f);
                  return h && t.openImagePreview?.(h, "RunningHub 结果");
                },
                children: m.jsx(cf, { size: 15 }),
              }),
              m.jsx("button", {
                type: "button",
                className: "compact-icon-btn",
                ...
                onClick: (f) => {
                  Lt(f);
                  return h && t.downloadImage?.(h, `runninghub-${Date.now()}.png`);
                },
                children: m.jsx(tC, { size: 15 }),
              }),
            ],
          })
        : null,
      xp
        ? m.jsxs("div", {
            ...
            children: [
              (t.upstreamImages || []).length
                ...
                : null,
              m.jsx("div", { className: "rh-node-label", children: "选择工作流（在 API 设置 → RunningHub 工作流 添加）" }),
              m.jsxs("div", {
                className: "compact-toolbar",
                children: [
                  m.jsxs("button", {
                    type: "button",
                    className: `compact-pill relative${rhOpen ? " active" : ""}`,
                    title: "工作流",
                    onClick: () => setRhOpen(!rhOpen),
                    children: [
                      m.jsx(oC, { size: 14 }),
                      m.jsx("span", { className: "select-label", children: "工作流" }),
                      m.jsx("span", { className: "select-val", children: rhCur ? rhCur.name || rhCur.workflowId : "未选择工作流" }),
                      m.jsx("span", { className: "select-arrow", children: "▾" }),
                      rhOpen
                        ? m.jsx(Rr, {
                            type: "genlist",
                            options: rhList.map((f) => ({ value: f.id, label: f.name || `工作流 ${f.workflowId}` })),
                            value: t.rhWorkflowRef || "",
                            onPick: (f) => t.updateNode?.(e, { rhWorkflowRef: f }),
                            onClose: () => setRhOpen(!1),
                          })
                        : null,
                    ],
        ...
        : null,
      m.jsx(lt, { type: "source", position: q.Right, className: "handle source plus-handle", children: m.jsx(mo, { size: 12 }) }),
    ],
  });
}
function aMCompact({ id: e, data: t, selected: n }) {
  const [r, o] = z.useState(null),
    [mt, st] = z.useState(!1),
    [xp, setXp] = z.useState(!1),
    [historyOpen, setHistoryOpen] = z.useState(!1),
    [imgRatio, setImgRatio] = z.useState(null),
    C = z.useRef(null),
    Fr = z.useRef(null),
    i = (f) => {
      st(!1);
    ...
    c = (f) => {
      var x;
      const p = Dt(s, f),
        A = Si(s, { ...t, platformId: f }, "generateNode");
      (x = t.updateNode) == null ||
        x.call(t, e, { platformId: f, model: kt(p).includes(A) ? A : gr(p), _modelPinned: !1 });
    },
    hasImg = !!t.image,
    history = t.outputImages || [],
    hasHistory = history.length > 1,
    h = t.image || (t.outputImages || [])[0] || "",
    y = (f, p) => {
      var x;
      const A = f.target.value;
      if (A.endsWith("@")) st(!0);
      else if (mt) st(!1);
      return (x = t.updateNode) == null ? void 0 : x.call(t, e, { [p]: A });
    },
    onPick = async (f) => {
      var x;
      const p = f && f[0];
      ...
      if (!p.type.startsWith("image/")) {
        (x = t.addLog) == null || x.call(t, "失败：请选择图片文件。", "error");
        return;
      }
      try {
        const A = await Gu(p);
        t.updateNode?.(e, { image: A, fileName: p.name, status: "已上传" });
      } catch (A) {
        t.addLog?.(`上传失败：${A.message}`, "error");
      }
    };
  z.useEffect(() => {
    if (!xp) return;
  ...
  const prevImageRef = z.useRef(t.image);
  z.useEffect(() => {
    if (prevImageRef.current !== t.image) {
      prevImageRef.current = t.image;
    }
  }, [t.image, t.running]);
  z.useEffect(() => {
    if (t.image && !t.running && t.status === "生成成功") setXp(!0);
  }, [t.image, t.running, t.status]);
  z.useEffect(() => {
    t.image || setImgRatio(null);
  }, [t.image]);
  return m.jsxs(Ai, {
    title: t.title || "图片",
    icon: fl,
    status: t.status,
    selected: n,
    headerRight: imgRatio ? `${imgRatio.w} × ${imgRatio.h}` : "",
    className: `image-node generate-node compact-node bare-image-node ${hasImg ? "has-image" : "no-image"}`,
    children: [
      m.jsx(lt, { type: "target", position: q.Left, className: "handle target" }),
      m.jsx("button", {
        type: "button",
        className: "compact-stage",
        style: imgRatio ? { aspectRatio: imgRatio.w + " / " + imgRatio.h } : undefined,
        title: hasImg
          ? xp
            ? "收起操作"
            : "展开操作"
        ...
        children: [
          hasImg
            ? m.jsx("img", {
                ...
                onLoad: (f) => {
                  const x = f.target;
                  x && x.naturalWidth && setImgRatio({ w: x.naturalWidth, h: x.naturalHeight });
                },
              })
            : m.jsx("span", {
                className: "compact-placeholder compact-placeholder-textonly",
                children: m.jsx("small", { children: "上传 / 生成图片" }),
              }),
          t.running ? m.jsx("span", { className: "compact-running", children: m.jsx(ao, { size: 14, className: "spin" }) }) : null,
        ],
      }),
      m.jsx("span", {
        className: `compact-upload-overlay nodrag nopan${hasImg ? "" : " centered"}`,
        title: hasImg ? "上传/替换图片" : "上传本地图片",
        role: "button",
        tabIndex: 0,
        onPointerDown: (f) => {
          f.preventDefault();
        ...
        children: m.jsx(Wu, { size: hasImg ? 15 : 20 }),
      }),
      xp && hasImg
        ? m.jsxs("div", {
            className: "compact-top nodrag nopan",
            children: [
              m.jsx("button", {
                ...
                onClick: (f) => {
                  Lt(f);
                  return h && t.openImagePreview?.(h, t.fileName || "图片");
                },
                children: m.jsx(cf, { size: 15 }),
              }),
              m.jsx("button", {
                ...
                onClick: (f) => {
                  Lt(f);
                  return h && t.downloadImage?.(h, t.fileName || "图片.png");
                },
                children: m.jsx(tC, { size: 15 }),
              }),
              hasHistory
                ? m.jsx("button", {
                    type: "button",
                    className: "compact-history-toggle nodrag nopan",
                    title: "查看生成历史",
                    "aria-label": "查看生成历史",
                    onPointerDown: Lt,
                    onMouseDown: Lt,
                    ...
                    children: m.jsx(nC, { size: 15 }),
                  })
                : null,
              hasHistory && historyOpen
                ? m.jsxs("div", {
                    className: "compact-history-popover nodrag nopan",
                    children: [
                      m.jsx("div", { className: "compact-history-title", children: "生成历史" }),
                      m.jsx("div", {
                        ...
                        children: (t.outputImages || []).map((f, p) =>
                          m.jsx(
                            ...
                            {
                              type: "button",
                              className: `compact-history-item${f === t.image ? " current" : ""}`,
                              title: `选择第 ${p + 1} 张图片`,
                              onClick: (A) => {
                                Lt(A);
                                t.updateNode?.(e, { image: f });
                                setHistoryOpen(!1);
                              },
                              children: m.jsx("img", { src: f, alt: `历史图片 ${p + 1}`, draggable: !1 }),
                            },
                            `compact-history-${p}`,
                          ),
                        ),
        ...
        ? m.jsxs("div", {
            ...
            children: [
              ...
              m.jsx("textarea", {
                ...
                onPointerDown: Lt,
                onMouseDown: Lt,
                onChange: (f) => y(f, "prompt"),
              }),
              mt
                ? m.jsxs("div", {
                    className: "compact-mention nodrag nopan",
                    children: [
                      m.jsx("div", { className: "mention-title", children: "引用上游图片" }),
                      (t.upstreamImages || []).length
                        ? (t.upstreamImages || []).map((f, p) =>
                            m.jsxs(
                              "button",
                              {
                                ...
                                onMouseDown: (A) => {
                                  A.preventDefault();
                                  st(!1);
                                  return t.updateNode?.(e, {
                                    prompt: (t.prompt || "").replace(/@$/, `图${p + 1}`),
                                  });
                                },
                                children: [
                                  m.jsx("img", { src: f, alt: "", draggable: !1 }),
              ...
              m.jsxs("div", {
                ...
                children: [
                  ...
                  m.jsxs("button", {
                    type: "button",
                    className: `compact-pill${r === "model" ? " active" : ""}`,
                    title: "模型",
                    onClick: () => i("model"),
                    children: [
                      m.jsx(vi, { size: 14 }),
                      m.jsx("span", { children: u || "选择模型" }),
                      r === "model"
                        ? m.jsx(Rr, {
                            type: "model",
                            options: d,
                  ...
                  m.jsxs("button", {
                    type: "button",
                    className: `compact-pill${r === "size" ? " active" : ""}`,
                    title: "尺寸与清晰度",
                    onClick: () => i("size"),
                    children: [
                      m.jsx(ey, { size: 14 }),
                      m.jsx("span", { children: `${t.ratio || "自适应"} · ${(t.quality || "1k").toUpperCase()}` }),
                      r === "size"
                        ? m.jsxs("div", {
                            className: "compact-size-popover nodrag nopan",
                            children: [
                              m.jsx("div", { className: "compact-popover-title", children: "清晰度" }),
                              m.jsx(Rr, {
                                type: "quality",
                                options: ["1k", "2k", "3k", "4k"],
                                value: t.quality || "1k",
                                onPick: (f) => t.updateNode?.(e, { quality: f }),
                                onClose: () => o(null),
                              }),
                              m.jsx("div", { className: "compact-popover-title", children: "比例" }),
                              m.jsx(Rr, {
                                type: "ratio",
                                options: [...pC, "自适应"],
                                value: t.ratio || "自适应",
                                onPick: (f) => t.updateNode?.(e, { ratio: f }),
                                onClose: () => o(null),
                              }),
                            ],
                          })
                  ...
                  m.jsxs("button", {
                    type: "button",
                    className: `compact-pill${r === "count" ? " active" : ""}`,
                    title: "生成张数",
                    onClick: () => i("count"),
                    children: [
                      m.jsx(eC, { size: 14 }),
                      m.jsx("span", { children: `${t.count || 1}张` }),
                      r === "count"
                        ? m.jsx(Rr, {
                            type: "count",
                            options: hC.map((f) => `${f}张`),
                            value: `${t.count || 1}张`,
                            onPick: (f) => t.updateNode?.(e, { count: Number(String(f).replace("张", "")) }),
                            onClose: () => o(null),
                          })
                        : null,
                    ],
      ...
      m.jsx("input", {
        ...
        accept: "image/*",
        hidden: !0,
        onChange: (f) => onPick(f.target.files),
      }),
      m.jsx(lt, { type: "source", position: q.Right, className: "handle source plus-handle", children: m.jsx(mo, { size: 12 }) }),
    ],
  });
}
function bMCompact({ id: e, data: t, selected: n }) {
  const [xp, setXp] = z.useState(!1),
    [mt, st] = z.useState(!1),
    [vidRatio, setVidRatio] = z.useState(null),
    [sizeOpen, setSizeOpen] = z.useState(!1),
    A = z.useRef(null),
    r = t.settings || tn,
    o = Kl(r, t.platformId || r.activePlatformId),
    i = (s, l) => t.updateNode?.(e, { [s]: l }),
    v = t.outputVideo || "",
    g = ["3:4", "4:3", "9:16", "16:6"].includes(t.size)
      ? t.size
      : { "1280x720": "4:3", "1920x1080": "4:3", "720x1280": "9:16", "1080x1920": "9:16" }[t.size] || "9:16";
  z.useEffect(() => {
    if (!xp) return;
    const s = (l) => {
      const a = l.target;
      const rhHost = a instanceof Element ? a.closest(".react-flow__node") : null;
      (!rhHost || rhHost.getAttribute("data-id") !== e) && setXp(!1);
    };
    return (
  ...
  z.useEffect(() => {
    ...
    const s = (l) => {
      const a = l.target;
      a instanceof Element && !a.closest(".compact-panel") && setSizeOpen(!1);
    };
    return (
      document.addEventListener("pointerdown", s),
      () => document.removeEventListener("pointerdown", s)
  ...
  return m.jsxs(Ai, {
    ...
    status: t.status,
    selected: n,
    className: "video-node compact-node",
    children: [
      m.jsx(lt, { type: "target", position: q.Left, className: "handle target" }),
      m.jsx("button", {
        type: "button",
        className: "compact-stage",
        style: vidRatio ? { aspectRatio: vidRatio.w + " / " + vidRatio.h } : undefined,
        title: xp ? "收起编辑面板" : "展开编辑面板",
        onPointerDown: (s) => {
          A.current = { x: s.clientX, y: s.clientY, moved: !1 };
        },
        onPointerMove: (s) => {
          A.current && s.buttons === 1 && Math.hypot(s.clientX - A.current.x, s.clientY - A.current.y) > 4 && (A.current.moved = !0);
        },
        onClick: (s) => {
          const l = A.current;
          if (l && (l.moved || Math.hypot(s.clientX - l.x, s.clientY - l.y) > 4)) {
            return;
          }
          setXp(!xp);
        },
        children: [
          v
            ? m.jsx("video", {
                ...
                onLoadedMetadata: (s) => {
                  const l = s.target;
                  l && l.videoWidth && setVidRatio({ w: l.videoWidth, h: l.videoHeight });
                },
              })
            : m.jsxs("span", {
                className: "compact-placeholder",
                children: [m.jsx(go, { size: 24 }), m.jsx("small", { children: t.outputVideoId ? "任务已提交" : "点击生成" })],
              }),
          t.running ? m.jsx("span", { className: "compact-running", children: m.jsx(ao, { size: 14, className: "spin" }) }) : null,
        ],
      }),
        ...
        ? m.jsxs("div", {
            ...
            children: [
              m.jsx("button", {
                ...
                onClick: (s) => {
                  Lt(s);
                  return v && window.open(v, "_blank", "noopener");
                },
                children: m.jsx(cf, { size: 15 }),
              }),
              m.jsx("button", {
                ...
                onClick: (s) => {
                  Lt(s);
                  return v && t.downloadImage?.(v, `生成视频-${t.outputVideoId || Date.now()}.mp4`);
                },
                children: m.jsx(tC, { size: 15 }),
              }),
            ],
      ...
      xp
        ? m.jsxs("div", {
            ...
            children: [
                            ...
                            m.jsxs(
                              "button",
                              {
                                ...
                                onMouseDown: (a) => {
                                  a.preventDefault();
                                  st(!1);
                                  return i("prompt", (t.prompt || "").replace(/@$/, `图${l + 1}`));
                                },
                                children: [
                                  m.jsx("img", { src: s, alt: "", draggable: !1 }),
                                  m.jsx("span", { children: `图 ${l + 1}` }),
                                ],
                              },
                              `compact-video-mention-${l}`,
                            ),
                          )
                        : m.jsx("div", { className: "mention-empty", children: "还没有上游图片，先从图片节点连线过来" }),
                    ],
              ...
              m.jsxs("div", {
                className: "compact-toolbar",
                children: [
                  m.jsx(wf, { settings: r, value: o, onChange: (s) => i("platformId", s) }),
              m.jsxs("button", {
                type: "button",
                className: `compact-pill relative${sizeOpen ? " active" : ""}`,
                title: "尺寸",
                onClick: () => setSizeOpen(!sizeOpen),
                children: [
                  m.jsx(ey, { size: 14 }),
                  m.jsx("span", { className: "select-label", children: "尺寸" }),
                  m.jsx("span", { className: "select-val", children: g }),
                  m.jsx("span", { className: "select-arrow", children: "▾" }),
                  sizeOpen
                    ? m.jsx(Rr, {
                        type: "genlist",
                        options: ["3:4", "4:3", "9:16", "16:6"],
                        value: g,
                        onPick: (s) => i("size", s),
                        onClose: () => setSizeOpen(!1),
                      })
                    : null,
                ],
              }),
        ...
        : null,
      m.jsx("label", {
        className: "compact-hidden-control",
        children: [
          "时长",
          m.jsx("select", {
            value: String(t.duration || "10"),
            onChange: (s) => i("duration", s.target.value),
            children: ["10"].map((s) => m.jsx("option", { value: s, children: `${s}s` }, s)),
          }),
        ],
      }),
      m.jsx("label", {
...
function dM({
  ...
  const [armed, setArmed] = z.useState(!1),
    rf = Vl();
  z.useEffect(() => {
    if (!armed) return;
    const f = (p) => {
      const x = p.target;
      (x && x.closest && x.closest(".edge-cut")) || setArmed(!1);
    };
    return (
      document.addEventListener("pointerdown", f),
      () => document.removeEventListener("pointerdown", f)
    ...
    children: [
      ...
      armed
        ? m.jsxs("g", {
            ...
            onClick: (f) => {
              f.stopPropagation();
              rf.setEdges((Q) => Q.filter((Q2) => Q2.id !== e));
            },
            children: [
              m.jsx("circle", { cx: a, cy: u, r: 13, className: "edge-cut-circle" }),
              m.jsx("text", {
                x: a,
                y: u,
                className: "edge-cut-icon",
                textAnchor: "middle",
  ...
  return e
    ? m.jsxs("div", {
        ...
        children: [
          ...
          m.jsxs("button", {
            type: "button",
            onClick: () => t("runningHubNode"),
            children: [m.jsx(oC, { size: 15 }), " RunningHub"],
          }),
          m.jsx("button", {
            type: "button",
            className: "ghost",
...
function mM({ preview: e, onClose: t }) {
  const [n, r] = z.useState({ x: 0, y: 0 }),
    [o, i] = z.useState(1),
    [size, setSize] = z.useState(null),
    s = z.useRef(null);
  z.useEffect(() => {
    if (!e) return;
    (r({ x: 0, y: 0 }), i(1), setSize(null));
  }, [e]);
  if (!e) return null;
  const zoomBy = (a) => i((c) => Math.max(0.05, Number((c + a).toFixed(2))));
  return m.jsxs("div", {
    className: "image-preview-overlay",
    onMouseDown: (a) => {
      if (a.target === a.currentTarget) t();
    },
    children: [
      m.jsxs("div", {
        ...
        children: [
          m.jsx("span", {
            ...
            children: e.name || "图片预览",
          }),
          size
            ? m.jsx("span", {
                className: "image-preview-pct",
                children: `${size.w}×${size.h}`,
              })
            : null,
          m.jsx("button", {
            type: "button",
            className: "image-preview-btn primary",
            title: "下载",
            onClick: () => {
              if (e.download) return e.download();
              const a = document.createElement("a");
              ((a.href = e.image), (a.download = e.name || "图片.png"), (a.rel = "noopener"), a.click());
            },
            children: "下载",
          }),
          m.jsx("button", {
          ...
          m.jsx("span", { className: "image-preview-pct", children: Math.round(o * 100) + "%" }),
          m.jsx("button", {
            type: "button",
            className: "image-preview-btn",
            title: "放大",
      ...
      m.jsx("div", {
        ...
        onWheel: (a) => {
          (a.preventDefault(), zoomBy(a.deltaY < 0 ? 0.12 : -0.12));
        },
        children: m.jsx("img", {
          src: e.image,
          alt: e.name || "图片预览",
          ...
          onLoad: (a) => {
            const u = a.target;
            u && u.naturalWidth && setSize({ w: u.naturalWidth, h: u.naturalHeight });
          },
          onMouseDown: (a) => {
            a.preventDefault();
            a.stopPropagation();
            const start = { x: a.clientX, y: a.clientY, ox: n.x, oy: n.y };
            const move = (c) => {
              const dx = c.clientX - start.x,
                dy = c.clientY - start.y;
              (Math.hypot(dx, dy) > 4 && r({ x: start.ox + dx, y: start.oy + dy }));
            };
            const up = () => {
...
function yM({
  ...
  const [i, s] = z.useState(""),
    [g, v] = z.useState([]),
    [h, w] = z.useState([]),
    [platformDragId, setPlatformDragId] = z.useState(null),
    [platformDragOverId, setPlatformDragOverId] = z.useState(null),
    [rhId, setRhId] = z.useState(null),
    l = Ar(n);
  if (!e) return null;
  const rhBaseCfg = n.runningHub || tn.runningHub,
    rhW = (rhBaseCfg.workflows || []).find((P) => P.id === rhId) || null,
    rhUpd = (P) => {
      r({
        ...n,
        runningHub: {
          ...rhBaseCfg,
          workflows: (rhBaseCfg.workflows || []).map((T) =>
            T.id === rhW.id ? { ...T, ...P } : T,
          ),
        },
      });
    ...
    rhAdd = () => {
      const P = {
        id: Ft("rhw"),
        name: `工作流 ${(rhBaseCfg.workflows || []).length + 1}`,
        workflowId: "",
        note: "",
        params: [],
      };
      (r({
        ...n,
        runningHub: { ...rhBaseCfg, workflows: [...(rhBaseCfg.workflows || []), P] },
      }),
        setRhId(P.id));
    },
    rhDel = () => {
      (r({
        ...
        runningHub: {
          ...rhBaseCfg,
          workflows: (rhBaseCfg.workflows || []).filter((T) => T.id !== rhW.id),
        },
      }),
        setRhId(null));
    };
  const a = (P) => {
    ...
    N = async () => {
      try {
        ...
        let F = null;
        try {
          const L = await co(P, { method: "GET", headers: Xu(l.apiKey) }, 3e4);
          F = await hy(L);
        } catch (ue) {
          F = null;
        }
        let E = F ? parseApiModels(F) : [];
        if (!E.length) {
          const Q = Ps(l.baseUrl).replace(/\/v1$/i, "") + "/v1/models";
          if (Q !== P) {
            s("正在尝试 /v1/models ...");
            const L2 = await co(Q, { method: "GET", headers: Xu(l.apiKey) }, 3e4);
            F = await hy(L2);
            E = parseApiModels(F);
          }
        }
        ...
        (v(E), w([]), s(`已获取 ${E.length} 个模型，请勾选后应用。`));
      } catch (P) {
        let L = `获取模型失败：${P.message}`;
        /failed to fetch|网络|network/i.test(String(P.message)) &&
          (L += "。可能是浏览器跨域拦截或网络问题，部署到 Cloudflare Pages 后会自动通过代理请求。");
    ...
    handlePlatformDragStart = (P, T) => {
      setPlatformDragId(T.id);
      P.dataTransfer.effectAllowed = "move";
      try {
        P.dataTransfer.setData("text/plain", T.id);
      } catch {}
    },
    movePlatformTo = (P, T) => {
      P.preventDefault();
      P.stopPropagation();
      setPlatformDragOverId(null);
      if (!platformDragId) return;
      const L = [...n.platforms],
        F = L.findIndex((E) => E.id === platformDragId);
      if (F < 0) return;
      if (T && platformDragId === T.id) return;
      const [E] = L.splice(F, 1),
        $ = T ? L.findIndex((G) => G.id === T.id) : L.length;
      L.splice($, 0, E);
      const G = kt(L[0]),
        W = Up(L[0]);
      setPlatformDragId(null);
      r({
        ...n,
        platforms: L,
        activePlatformId: L[0].id,
        defaultImageModel: G.includes(n.defaultImageModel)
          ? n.defaultImageModel
          : G[0] || "",
        defaultTextModel: W.includes(n.defaultTextModel)
          ? n.defaultTextModel
          : W[0] || "",
      });
      q && n.platforms[0] && n.platforms[0].id !== L[0].id && q(L[0].id);
    },
          ...
          children: [
            m.jsxs("div", {
              ...
              onDragLeave: (L) => {
                ...
                (!R || !L.currentTarget.contains(R)) &&
                  setPlatformDragOverId(null);
              },
              onDrop: (L) => movePlatformTo(L, null),
              children: [
                m.jsx("div", {
                  className: "side-title",
                  children: "API 地址（可拖动排序）",
                }),
                n.platforms.map((P) =>
                  m.jsxs(
                    "button",
                    {
                      className: [
                        P.id === n.activePlatformId ? "active" : "",
                        platformDragId === P.id ? "dragging" : "",
                        platformDragOverId === P.id ? "drag-over" : "",
                      ]
                        .filter(Boolean)
                        .join(" "),
                      type: "button",
                      draggable: !0,
                      onDragStart: (L) => handlePlatformDragStart(L, P),
                      onDragEnter: () => setPlatformDragOverId(P.id),
                      onDragOver: (L) => {
                        L.preventDefault();
                        L.dataTransfer && (L.dataTransfer.dropEffect = "move");
                        setPlatformDragOverId(P.id);
                      },
                      onDrop: (L) => movePlatformTo(L, P),
                ...
                m.jsx("div", {
                  className: "side-title rh-side-title",
                  children: "RunningHub 工作流",
                }),
                (rhBaseCfg.workflows || []).map((P) =>
                  m.jsxs(
                    "button",
                    {
                      type: "button",
                      className: P.id === rhId ? "active" : "",
                      title: P.note || P.name || "",
                      onClick: () => setRhId(P.id),
                      children: [
                        m.jsx(oC, { size: 14 }),
                        m.jsx("span", {
                          className: "rh-wf-name",
                          children: P.name || "未命名工作流",
                        }),
                      ],
                    },
                    P.id,
                ...
                m.jsx("button", {
                  ...
                  className: "add-platform",
                  onClick: rhAdd,
                  children: [m.jsx(mo, { size: 14 }), " 添加工作流"],
                }),
              ],
            }),
            m.jsxs("div", {
              ...
              children: rhW
                ? [
                    m.jsxs("div", {
                      className: "rh-settings-head",
                      children: [
                        m.jsx(oC, { size: 15 }),
                        m.jsx("b", { children: "RunningHub 工作流配置" }),
                      ],
                    }),
                    m.jsxs("label", {
                      children: [
                        "API 地址（所有 RunningHub 工作流共用）",
                        m.jsx("input", {
                          value: rhBaseCfg.baseUrl || "",
                          placeholder: "填写 RunningHub API 地址",
                          onChange: (P) =>
                            r({
                              ...n,
                              runningHub: { ...rhBaseCfg, baseUrl: P.target.value },
                            }),
                        }),
                      ],
                    }),
                    m.jsxs("label", {
                      children: [
                        ...
                        m.jsx("input", {
                          type: "password",
                          value: rhBaseCfg.apiKey || "",
                          placeholder: "RunningHub 的 32 位 API Key",
                          onChange: (P) =>
                            r({
                              ...n,
                              runningHub: { ...rhBaseCfg, apiKey: P.target.value },
                            }),
                        }),
                      ],
                    }),
                    m.jsxs("label", {
                      children: [
                        ...
                        m.jsx("input", {
                          value: rhW.name || "",
                          placeholder: "例如：SeedVR2 高清放大",
                          onChange: (P) => rhUpd({ name: P.target.value }),
                        }),
                      ],
                    }),
                    m.jsxs("label", {
                      children: [
                        "工作流 ID",
                        m.jsx("input", {
                          value: rhW.workflowId || "",
                          placeholder: "例如：20588248549437850625",
                          onChange: (P) => rhUpd({ workflowId: P.target.value }),
                        }),
                      ],
                    }),
                    m.jsx("div", {
                      className: "rh-wf-path",
                      children: `# /run/workflow/${rhW.workflowId || "…"}`,
                    }),
                    m.jsxs("label", {
                      children: [
                        "备注、用途、参数说明",
                        m.jsx("textarea", {
                          ...
                          rows: 3,
                          value: rhW.note || "",
                          placeholder: "备注、用途、参数说明",
                          onChange: (P) => rhUpd({ note: P.target.value }),
                        }),
                      ],
                    }),
                    m.jsxs("div", {
                      className: "rh-param-head",
                      children: [
                        "参数映射（图片字段按顺序取节点上游图）",
                        m.jsx("button", {
                          type: "button",
                          className: "rh-add-param",
                          onClick: () =>
                            rhUpd({
                              params: [
                                ...(rhW.params || []),
                                { type: "text", nodeId: "", fieldName: "", value: "" },
                              ],
                            }),
                          children: "+ 添加参数",
                        }),
                    ...
                    ...(rhW.params || []).length
                      ? (rhW.params || []).map((P, T) =>
                          m.jsxs(
                            "div",
                            {
                              className: "rh-field-row",
                              children: [
                                m.jsxs("select", {
                                  className: "rh-node-input rh-field-type",
                                  value: P.type || "text",
                                  onChange: (L) =>
                                    rhUpd({
                                      params: (rhW.params || []).map((A, B) =>
                                        B === T ? { ...P, type: L.target.value } : A,
                                      ),
                                    }),
                                  children: [
                                    m.jsx("option", { value: "text", children: "文本" }),
                                    m.jsx("option", { value: "image", children: "图片" }),
                                  ],
                                }),
                                m.jsx("input", {
                                  className: "rh-node-input rh-field-nodeid",
                                  value: P.nodeId || "",
                                  placeholder: "节点ID",
                                  onChange: (L) =>
                                    rhUpd({
                                      params: (rhW.params || []).map((A, B) =>
                                        B === T ? { ...P, nodeId: L.target.value } : A,
                                      ),
                                    }),
                                }),
                                m.jsx("input", {
                                  className: "rh-node-input",
                                  value: P.fieldName || "",
                                  placeholder: "字段名",
                                  onChange: (L) =>
                                    rhUpd({
                                      params: (rhW.params || []).map((A, B) =>
                                        B === T ? { ...P, fieldName: L.target.value } : A,
                                      ),
                                    }),
                                }),
                                P.type === "image"
                                  ? m.jsx("span", {
                                      className: "rh-field-idx",
                                      children: `上游图${(rhW.params || []).filter((A, B) => A.type === "image" && B < T).length + 1}`,
                                    })
                                  : m.jsx("input", {
                                      className: "rh-node-input",
                                      value: P.value || "",
                                      ...
                                      onChange: (L) =>
                                        rhUpd({
                                          params: (rhW.params || []).map((A, B) =>
                                            B === T ? { ...P, value: L.target.value } : A,
                                          ),
                                        }),
                                    }),
                                m.jsx("button", {
                                  type: "button",
                                  className: "rh-field-del",
                                  title: "删除该参数",
                                  onClick: () =>
                                    rhUpd({
                                      params: (rhW.params || []).filter((A, B) => B !== T),
                                    }),
                                  children: "×",
                                }),
                              ],
                      ...
                      : [
                          m.jsx("div", {
                            className: "mention-empty",
                            children: "不需要改参数可留空，直接用工作流默认值运行",
                          }),
                        ],
                    m.jsxs("div", {
                      ...
                      children: [
                        m.jsxs("button", {
                          ...
                          className: "danger",
                          onClick: rhDel,
                          children: [m.jsx(aC, { size: 15 }), " 删除该工作流"],
                        }),
                      ],
                    }),
                    m.jsx("div", {
                      ...
                      children:
                        "点上方「保存设置」持久化。画布里添加 RunningHub 节点，在二级菜单选择此工作流即可运行。",
                    }),
                  ]
                : [
                m.jsxs("label", {
...
function vM() {
  ...
  const e = Vl(),
    ...
    applyTopPlatformToNodes = z.useCallback(
      (O) => {
        if (!O) return;
        o((U) =>
          U.map((V) => {
            if (
              !["generateNode", "videoNode", "textNode"].includes(V.type) ||
              V.data.platformId === O
            )
              return V;
            return { ...V, data: { ...V.data, platformId: O, _modelPinned: !1 } };
          }),
        );
      },
      [o, t],
    ),
    [selBox, setSelBox] = z.useState(null),
    [selLine, setSelLine] = z.useState(null),
    [snapGuides, setSnapGuides] = z.useState(null),
    rhCleanupFx = z.useEffect(() => {
      o((O) => {
        const U = O.filter(
          (V) => !["angleNode", "upscaleNode", "reverseNode"].includes(V.type),
        );
        return U.length === O.length ? O : U;
      });
    }, [o]),
    selFx = z.useEffect(() => {
      const update = () => {
        ...
        if (vp) {
          const m2 = /scale\(([\d.]+)\)/.exec(vp.style.transform || "");
          const zz = m2 ? parseFloat(m2[1]) : 1;
          document.documentElement.style.setProperty(
            "--node-zoom",
            String(Math.min(2, Math.max(1, 1 / (zz || 1)))),
          );
        }
        const els = [
          ...document.querySelectorAll(".react-flow__node.selected"),
        ].filter((V) => !V.querySelector(".group-node"));
        if (els.length < 2) {
          setSelBox(null);
          return;
        }
        ...
        els.forEach((V) => {
          ...
          (x1 = Math.min(x1, r0.x)),
            (y1 = Math.min(y1, r0.y)),
            (x2 = Math.max(x2, r0.right)),
            (y2 = Math.max(y2, r0.bottom));
        });
        setSelBox({ x: x1 - 18, y: y1 - 18, w: x2 - x1 + 36, h: y2 - y1 + 36 });
      };
      update();
      const vpEl = document.querySelector(".react-flow__viewport");
      const mo2 = vpEl ? new MutationObserver(update) : null;
      mo2 && mo2.observe(vpEl, { attributes: !0, attributeFilter: ["style"] });
    ...
    onPlusDown = z.useCallback(
      (O) => {
        O.stopPropagation();
        O.preventDefault();
        const move = (B) => setSelLine({ x2: B.clientX, y2: B.clientY });
        const up = (B) => {
          window.removeEventListener("pointermove", move);
          window.removeEventListener("pointerup", up);
          setSelLine(null);
          const el = document.elementFromPoint(B.clientX, B.clientY);
          ...
          if (targetId) {
                  ...
                  Ou(
                    {
                      ...
                      target: targetId,
                      type: "customEdge",
                      markerEnd: { type: sn.ArrowClosed, width: 18 },
                      style: { strokeWidth: 2 },
                    },
                    acc,
                  ),
            ...
            k(`已连接 ${ids.length} 个选中节点到目标节点。`, "success");
          } else {
            const B2 = e.screenToFlowPosition({ x: B.clientX, y: B.clientY });
            d({ flow: B2, screen: { x: B.clientX, y: B.clientY }, sourceNodeIds: ids });
          }
        };
        window.addEventListener("pointermove", move);
        window.addEventListener("pointerup", up);
        setSelLine({ x2: O.clientX, y2: O.clientY });
      },
      [e, d, l, k],
    ),
    onNodesSnap = z.useCallback(
      (O) => {
        const posDrag = O.filter((Q) => Q.type === "position" && Q.dragging && Q.position);
        if (O.some((Q) => Q.type === "position" && Q.dragging === !1)) setSnapGuides(null);
        if (!posDrag.length || posDrag.length > 1) {
          i(O);
          return;
        }
        const c = posDrag[0],
          node = r.find((Q) => Q.id === c.id);
        if (!node) {
          i(O);
          return;
        }
        const boxOf = (Q) => {
            ...
            const st = el && el.querySelector ? el.querySelector(".compact-stage") : null;
            if (el && st) {
              const wr = el.getBoundingClientRect(),
                sr = st.getBoundingClientRect();
              return { ox: sr.x - wr.x, oy: sr.y - wr.y, w: sr.width, h: sr.height };
            }
            return {
              ox: 0,
              oy: 0,
              w: (Q.measured && Q.measured.width) || Q.width || 260,
              h: (Q.measured && Q.measured.height) || Q.height || 200,
            };
          },
          cb = boxOf(node),
          refs = r.filter((Q) => Q.id !== c.id && !Q.parentId),
          tol = 7;
        let bestX = tol,
          bestY = tol,
          snapX = 0,
        ...
        for (const o of refs) {
          const ob = boxOf(o),
            oX = o.position.x + ob.ox,
            oY = o.position.y + ob.oy,
            mX = c.position.x + cb.ox,
            mY = c.position.y + cb.oy;
          [
            [oX, mX],
            [oX + ob.w / 2, mX + cb.w / 2],
            [oX + ob.w, mX + cb.w],
          ].forEach(([oV, mV]) => {
            const dd = Math.abs(oV - mV);
            if (dd < bestX) ((bestX = dd), (snapX = oV - mV), (gx = oV));
          });
          [
            [oY, mY],
            [oY + ob.h / 2, mY + cb.h / 2],
            ...
            const dd = Math.abs(oV - mV);
            if (dd < bestY) ((bestY = dd), (snapY = oV - mV), (gy = oV));
          });
        }
        if (gx == null && gy == null) {
          setSnapGuides(null);
          ...
          O.map((Q) =>
            Q === c
              ? {
                  ...
                  position: {
                    ...Q.position,
                    x: Q.position.x + (gx != null ? snapX : 0),
                    y: Q.position.y + (gy != null ? snapY : 0),
                  },
                }
              : Q,
          ),
        ...
        setSnapGuides({
          v: gx != null ? e.flowToScreenPosition({ x: gx, y: c.position.y }).x : null,
          h: gy != null ? e.flowToScreenPosition({ x: c.position.x, y: gy }).y : null,
        });
      },
      [r, i, e],
    ),
    ...
    P = z.useCallback((O, B = "图片预览") => {
      O && N({ image: O, name: B, download: () => R(O, B) });
    }, [R]),
    Y = z.useCallback(
      (O, B) => {
        const U = e.getNodes(),
        ...
        for (const W of V) {
          ...
          if (
            (Jt.type === "generateNode" || Jt.type === "runningHubNode") &&
    ...
    F = z.useCallback(
      (O, B, U = {}) => {
        const typeLabels = {
            generateNode: "图片节点",
            videoNode: "视频节点",
            runningHubNode: "RunningHub节点",
            textNode: "文本节点",
          },
          nextIndex = r.filter((Q) => Q.type === O).length + 1,
          defaultTitle = `${typeLabels[O] || "节点"}${nextIndex}`,
          V = Ft(O),
          ...
          J = {
            ...
            textNode: {
              ...
              text: "",
              platformId: t.activePlatformId,
              model: Up(Ar(t)).includes(t.defaultTextModel)
                ? t.defaultTextModel
                : Up(Ar(t))[0] || "",
              upstreamImages: [],
              status: "等待运行",
        ...
        return (
          ...
          ((u && u.sourceNodeIds) || (u && u.sourceNodeId ? [u.sourceNodeId] : [])).length &&
            l((Q) =>
              ((u && u.sourceNodeIds) || (u && u.sourceNodeId ? [u.sourceNodeId] : [])).reduce(
                (acc, sid) =>
                  Ou(
                    {
                      id: Ft("edge"),
      ...
      [r, u, l, o, t.activePlatformId, t.defaultImageModel, t.defaultTextModel, L],
    ),
    Z = z.useEffect(() => {
      window.__AI2_CANVAS_BRIDGE = {
          ...
          load: (O) => {
            try {
              (o(
                ((O && O.nodes) || []).filter(
                  (W) =>
                    !["angleNode", "upscaleNode", "reverseNode"].includes(W.type),
                ),
              ),
              l((O && O.edges) || []),
        ...
        actions: {
          createNode: (O) => {
            try {
              return F(
                (O && O.type) || "generateNode",
                (O && O.position) || null,
                (O && O.data) || {},
              );
            } catch (B) {
              return { error: String((B && B.message) || B) };
            }
          },
          updateNode: (O, T) => {
            const Q = e.getNodes().find((U) => U.id === O);
            if (!Q) return { error: `未找到节点：${O}` };
            j(O, T || {});
            return { id: O };
          },
          connect: (O, T) =>
          ...
          runNode: (O) => {
            var Q;
            return (Q = C.current) == null ? void 0 : Q.call(C, O);
          },
          moveNode: (O, T, P) =>
            o((Q) =>
              Q.map((ue) =>
                ue.id === O ? { ...ue, position: { x: T, y: P } } : ue,
              ),
            ),
        },
      };
    ...
    E = z.useCallback(
      (O, B = e.getNodes(), U = e.getEdges()) => {
            ...
            (le.add(ue),
              U.filter((ze) => ze.target === ue).forEach((ze) => {
                ...
                if (je) {
                  ...
                  if (
                    (je.type === "generateNode" || je.type === "runningHubNode") &&
                    (((Zt = je.data.outputImages) != null && Zt.length) || je.data.image)
                  ) {
                    if (je.type === "generateNode") {
                      W.push(je.data.image || (je.data.outputImages || [])[0]);
                    } else {
                      W.push(...(je.data.outputImages || []));
                      je.data.image && W.push(je.data.image);
                    }
                    return;
                  }
                  (je.type === "textNode" &&
        ...
        return (
          Q(O),
          {
            images: W.map((Qe) => Gl(Qe)).filter(Boolean),
...
`),
  ...
  (z.useEffect(() => {
    o((O) => {
      let B = !1;
      const U = O.map((V) => {
        if (!["generateNode", "videoNode", "runningHubNode", "textNode"].includes(V.type))
          return V;
    ...
    z.useEffect(() => {
      o((O) => {
        let B = !1;
        const U = O.map((V) => {
          if (!["generateNode"].includes(V.type)) return V;
    ...
    D = z.useCallback(
      async (O) => {
        ...
        if (V) {
          ...
          try {
            if (
              !["generateNode", "videoNode", "runningHubNode", "textNode"].includes(
                V.type,
              )
            ) {
              k("这个节点不需要运行。", "info");
                ...
                Object.assign(
                  {
                    running: !1,
                    status: "生成成功",
                    outputImages: Array.from(new Set([...(V.data.outputImages || []), ...ue])),
                  },
            ...
            if (V.type === "runningHubNode") {
              const ue = await rHRunWorkflow({
                settings: t,
                nodeData: V.data,
                upstreamImages: J.images,
                onRequest: (ce) => k(`RunningHub：${ce}`, "info"),
                addLog: k,
              });
              (j(O, { running: !1, status: "运行成功", outputImages: ue }),
                $(V, ue),
                k(
                  `RunningHub 工作流完成：得到 ${ue.length} 个输出文件，已生成结果图片节点。`,
                  "success",
                ));
            }
            if (V.type === "textNode") {
              const ue = Dt(t, V.data.platformId);
              if (!(ue != null && ue.apiKey))
                throw new Error(
                  `请先在右上角「API 设置」给「${yf(ue)}」填写 API 密钥。`,
                );
              if (!V.data.model)
                throw new Error("请先在文本节点的二级菜单里选择文本模型。");
              const Q = await runTextChat({
                platform: ue,
                model: V.data.model,
                prompt: String(V.data.prompt || "").trim() || J.text || "",
                onRequest: (ce) => k(`实际请求：${ce}`, "info"),
              });
              (j(O, { running: !1, status: "生成成功", text: Q }),
                k("文本生成成功，结果已写入节点。", "success"));
            }
            return { ok: !0, id: O };
          } catch (J) {
      ...
      if (!x) {
        ...
        try {
          const O = e.getNodes(),
            B = e.getEdges(),
            U = O.filter((V) =>
              ["generateNode", "videoNode", "runningHubNode", "textNode"].includes(V.type),
            );
      ...
      async (O) => {
        const B = e.getNodes(),
          ...
          V = B.filter(
            (W) =>
              W.parentId === O &&
              ["generateNode", "videoNode", "runningHubNode", "textNode"].includes(W.type),
          );
        await I(V, U, "这个分组里没有可运行的节点。");
      },
      [e, I],
    ),
    b = z.useRef(H);
  ...
  const K = z.useCallback(
    ...
    te = z.useCallback(
      (O) => {
        ...
        const B = g.current,
          ...
          dropNodeEl =
            (W && W.closest && W.closest(".react-flow__node")) ||
            (U != null && document.elementFromPoint(U, V)
              ? document.elementFromPoint(U, V).closest(".react-flow__node")
              : null),
          dropNodeId = dropNodeEl ? dropNodeEl.getAttribute("data-id") : null;
        if (
          !v.current &&
          B != null &&
          B.nodeId &&
          ...
          const edgeDef =
            (B == null ? void 0 : B.handleType) === "target"
              ? { source: dropNodeId, target: B.nodeId }
              : { source: B.nodeId, target: dropNodeId };
          (l((Q) =>
            Ou(
              {
                id: Ft("edge"),
    ...
    re = z.useCallback(
      async (O) => {
          ...
          for (let W = 0; W < U.length; W += 1)
            try {
              ...
              F(
                ...
                { image: J, fileName: U[W].name, status: "已导入" },
              );
            } catch (J) {
        ...
        if (V && /^(https?:\/\/|data:image)/.test(V)) {
          F("generateNode", B, { image: V.trim(), fileName: "外部图片", status: "已导入" });
          return;
        }
        ...
        for (let V = 0; V < B.length; V += 1)
          try {
            ...
            F(
              ...
              { image: W, fileName: B[V].name, status: "已导入" },
            );
          } catch (W) {
          ...
          return (
            ...
            L({
              ...
              data: {
                ...W.data,
                status:
                  W.type === "imageNode" || W.type === "generateNode"
                    ? W.data.status
                    : "已粘贴",
              },
            })
    ...
    ce = z.useCallback(
      async (O) => {
        const B = O.target && O.target.tagName ? O.target.tagName.toLowerCase() : "";
        if (["input", "textarea", "select"].includes(B) || O.target?.isContentEditable) return;
        const U = O.clipboardData,
          V = Array.from((U && U.items) || [])
            .filter((W) => W.kind === "file" && W.type.startsWith("image/"))
            .map((W) => W.getAsFile())
            .filter(Boolean),
          J = V.length ? V : Array.from((U && U.files) || []).filter((W) => W.type.startsWith("image/"));
        if (J.length) {
          O.preventDefault();
          O.stopPropagation();
          const W = e.screenToFlowPosition({ x: window.innerWidth / 2, y: window.innerHeight / 2 });
          for (let Q = 0; Q < J.length; Q += 1)
            try {
              const ue = await Gu(J[Q]);
              F(
                "generateNode",
                { x: W.x + Q * 240, y: W.y + Q * 30 },
                { image: ue, fileName: `粘贴图片-${Date.now()}-${Q + 1}.png`, status: "已导入" },
              );
            } catch (ue) {
              k(`粘贴图片失败：${ue.message}`, "error");
            }
          return;
        }
        if (w.current.nodes.length) {
          O.preventDefault();
          pe();
        }
      },
    ...
    const O = (B) => {
      ...
      const U =
        (le = (J = B.target) == null ? void 0 : J.tagName) == null
          ? void 0
          : le.toLowerCase();
      const q1 = ["input", "textarea", "select"].includes(U) ||
        ((Q = B.target) == null ? void 0 : Q.isContentEditable);
      if (q1) {
        const el = B.target;
        const emptyField =
          el && (el.value != null ? el.value.trim() === "" : String(el.textContent || "").trim() === "");
        if (!(emptyField && (B.key === "Delete" || B.key === "Backspace"))) return;
        el && el.blur && el.blur();
      }
  ...
  return m.jsxs("div", {
    ...
    children: [
      ...
      m.jsxs("main", {
        ...
        children: [
          ...
          m.jsxs(gN, {
            ...
            onDrop: re,
            onDragOver: fe,
            onNodeDragStart: () => document.body.classList.add("node-dragging"),
            onNodeDragStop: () => document.body.classList.remove("node-dragging"),
            nodeTypes: Vn,
            edgeTypes: Bn,
            fitView: !0,
            selectionOnDrag: !0,
      ...
      snapGuides && snapGuides.v != null
        ? m.jsx("div", { className: "snap-guide snap-guide-v", style: { left: snapGuides.v } })
        : null,
      snapGuides && snapGuides.h != null
        ? m.jsx("div", { className: "snap-guide snap-guide-h", style: { top: snapGuides.h } })
        : null,
      selBox
        ? m.jsx("div", {
            className: "multi-sel-box",
            style: { left: selBox.x, top: selBox.y, width: selBox.w, height: selBox.h },
            children: m.jsx("button", {
              type: "button",
              className: "multi-sel-plus",
              title: "拖到节点或空白处，把全部选中节点连接过去",
              onPointerDown: onPlusDown,
              onDoubleClick: (O) => O.stopPropagation(),
              children: m.jsx(mo, { size: 16 }),
            }),
          })
        : null,
      selLine && selBox
        ? m.jsx(
            "svg",
            {
              className: "multi-sel-line",
              children: m.jsx("line", {
                x1: selBox.x + selBox.w,
                y1: selBox.y + selBox.h / 2,
                x2: selLine.x2,
                y2: selLine.y2,
              }),
            },
      ...
      m.jsx(yM, {
        ...
        setSettings: M,
        addLog: k,
        onPlatformReorder: applyTopPlatformToNodes,
      }),
    ],
  });
}
