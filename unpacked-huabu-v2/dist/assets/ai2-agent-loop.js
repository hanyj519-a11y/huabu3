...
const SPEC_SYSTEM_PROMPT = [
  ...
  "4. action 每轮只能包含一个动作；不要一次返回动作数组。action 必须使用 {name:string,...}，例如 {name:\"createNode\",alias:\"dog\",nodeType:\"generateNode\",data:{prompt:\"一只狗\"}}；运行节点必须使用 target:{alias:\"dogCat\"}（targetAlias/node/id 也会兼容转换）。",
  "5. 当 taskState.phase=planning 时，首次决策必须同时包含 taskPlan。执行器先校验并登记 taskPlan，再执行本轮 action。",
  "6. taskPlan 必须严格使用以下结构：{goal:string,steps:[{stepId:string,alias:string,intent:string,dependsOn:string[]}],successCriteria:[{alias:string,terminalStatus:\"success\",requireOutput:boolean} 或 {edge:{from:string,to:string,inputRole:\"image\"}}]}。steps 中每项必须是对象；dependsOn 可以引用后面声明的 alias，但不能引用不存在的 alias。",
  "7. reason 是给执行器日志使用的一句话，不能宣称尚未验证的结果。",
  "",
  "六、不可违反的规则",
...
function registerPlan(task, plan, opts) {
  ...
  /* 允许模型按自然语言顺序返回步骤；依赖可以引用后面才声明的 alias。 */
  for (const step of steps) {
    for (const dep of step.dependsOn) {
      if (!aliasSet.has(dep)) return { ok: false, error: "步骤 " + step.alias + " 依赖未定义的 alias：" + dep };
    }
  }
...
function validateDecision(decision, ctx) {
  ...
  if (type === "action") {
    ...
    /* 部分模型会把单动作名称写成 action.action；只兼容这一种等价字段，仍然禁止动作数组。 */
    const actionName = action.name || action.action;
    if (!actionName) {
      return { ok: false, kind: "invalid", error: "action.name 缺失：请在 action 对象中提供单个合法动作名称" };
    }
    if (ALLOWED_ACTION_NAMES.indexOf(actionName) === -1) {
      return { ok: false, kind: "invalid", error: "不支持或越权的动作：" + actionName };
    }
    if (!action.name) action.name = actionName;
    /* 统一模型常见的单节点引用字段，执行器内部只使用 target。 */
    if (!action.target && action.targetAlias != null) action.target = action.targetAlias;
    if (!action.target && action.node != null) action.target = action.node;
    if (!action.target && action.id != null) action.target = action.id;
...
async function runAgentTask(opts) {
  ...
  for (let turn = 1; turn <= MAX_TURNS; turn += 1) {
    ...
    if (!validation.ok) {
      /* 一次格式修复重问；仍失败则返回安全回退信号，面板不得执行旧版 actions */
      const repairInstruction = validation.error && /taskPlan|successCriteria|alias/.test(validation.error)
        ? "上一条回复的 taskPlan 不合法。请重新生成完整的首轮决策 JSON，不要只修改局部字段。taskPlan 必须严格为 {goal:string,steps:[{stepId:string,alias:string,intent:string,dependsOn:string[]}],successCriteria:[{alias:string,terminalStatus:\"success\",requireOutput:boolean} 或 {edge:{from:string,to:string,inputRole:\"image\"}}]}；successCriteria 中每一个 alias、edge.from、edge.to 都必须逐字复制 taskPlan.steps 中已经声明的 alias，不能写未登记的 dog、source 等别名，也不能使用节点标题代替 alias。步骤可以按任意顺序声明，但 dependsOn 只能引用 steps 中存在的 alias。首次 action 只能是一个 createNode，并使用其中一个已声明 alias。只输出 JSON，不要 Markdown 或解释。"
        : validation.kind === "legacy"
        ? "你返回了旧版 actions 数组。当前画布 Agent 禁止执行旧协议，尤其不能一次性创建多个节点。请把原任务改写为状态驱动循环：只输出一个合法 JSON 决策对象；顶层必须含 decisionId、type、reason；type 只能是 action、wait、clarify、finish；本轮只能有一个 action。若任务包含先生成、等待完成、再基于上游生成，首次决策必须含完整 taskPlan（每个步骤有唯一 alias 和 dependsOn），随后严格按 createNode → runNode → wait → connect → 下游 runNode → wait → finish 分轮返回。不要输出 actions 数组、Markdown 或解释。"
        : "上一条回复不符合决策协议。请只输出一个合法 JSON 决策对象；顶层含 decisionId、type、reason；type 只能是 action、wait、clarify、finish；当 type=action 时必须有 action.name，且 action 只能是一个对象，例如 {\"name\":\"createNode\",\"alias\":\"dog\",\"nodeType\":\"generateNode\",\"data\":{\"prompt\":\"一只狗\"}}；不要使用 action.action、actions 数组、Markdown 代码围栏或解释。";
      const repairMessages = messages.concat([
        { role: "assistant", content: typeof content === "string" ? content.slice(0, 4000) : "" },
        { role: "user", content: repairInstruction + " 原因：" + validation.error },
      ]);
      let repairedContent;
      try {
