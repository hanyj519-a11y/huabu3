...
(function () {
  ...
  var ddOpen = null;
  var activeTask = null; /* 循环编排任务（clarify 暂停后靠下一条用户消息续跑） */
  ...
  async function send() {
    ...
    try {
      if (window.__AI2_AGENT_LOOP && typeof window.__AI2_AGENT_LOOP.runAgentTask === "function") {
        await loopSend(b, plat, model, skill, text);
      } else {
        messages[messages.length - 1] = {
          role: "assistant",
          content: "❌ Agent 编排模块未加载，未执行任何画布动作。请刷新页面后重试。",
          details: ["__AI2_AGENT_LOOP 不可用，已阻止旧版一次性 actions"],
        };
        saveSession();
        renderMsgs();
      }
  ...
  /* 旧协议解析器仅保留给历史数据兼容；正常 Agent 入口禁止调用，避免绕过循环校验。 */
  async function legacySend(b, plat, model, skill, text, presetContent, extraDetails) {
    var parsed = extractActions(typeof presetContent === "string" ? presetContent : "");
    try {
      if (!parsed.actions.length) {
        ...
        parsed = extractActions(typeof content === "string" ? content : JSON.stringify(content));
      }
      ...
      messages[messages.length - 1] = {
        role: "assistant",
        content: "❌ " + ((e && e.message) || e),
        details: (extraDetails || []).concat(["处理失败：" + ((e && e.message) || e)]),
      };
      saveSession();
      renderMsgs();
    }
  ...
  /* RunningHub 注册表（规范化后供 Agent 校验与执行使用） */
  function getRhWorkflows() {
    var A = window.__AI2_RH_ADAPTER;
    var b = bridge();
    var settings = (b && b.get && b.get().settings) || {};
    var workflows = (settings.runningHub && settings.runningHub.workflows) || [];
    return A ? workflows.map(function (w) { return A.normalizeRegistryEntry(w); }).filter(Boolean) : [];
  }

  /* 新协议：状态驱动循环编排（GLM 5.3 Flash 无限画布 Agent 编排规范）
     每轮一个决策（action/wait/clarify/finish），执行器负责 alias 映射、waitFor 轮询与失败阻断。
     循环协议失败时必须停在安全边界；旧版 actions 不能进入一次性执行器，否则会绕过连线、等待和依赖校验。 */
  async function loopSend(b, plat, model, skill, text) {
    var LOOP = window.__AI2_AGENT_LOOP;
    var task;
    if (activeTask && activeTask.awaitingClarify) {
      LOOP.appendConversation(activeTask, text);
      task = activeTask;
    } else {
      task = LOOP.createEmptyTask(text);
      activeTask = task;
    }
    var callModel = async function (msgs) {
      var res = await fetch(proxyWrap(plat.baseUrl.replace(/\/+$/, "") + "/chat/completions"), {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + plat.apiKey },
        body: JSON.stringify({ model: model, stream: false, temperature: 0.2, messages: msgs }),
      });
      var raw = await res.text();
      var data;
      try { data = JSON.parse(raw); } catch (e) { throw new Error("返回无法解析：" + raw.slice(0, 200)); }
    ...
    var onTurn = function () {
      ...
      m.details = task.turns.map(function (t) { return LOOP.formatTurn(t); });
      renderMsgs();
    };
    var result = await LOOP.runAgentTask({
      bridge: b,
      callModel: callModel,
      task: task,
      onTurn: onTurn,
      deps: {
        ...
        getRhWorkflows: getRhWorkflows,
        rhRuntime: window.__AI2_RH_RUNTIME || null,
      },
    });
    var detailLines = task.turns.map(function (t) { return LOOP.formatTurn(t); });
    if (result.status === "completed") {
      activeTask = null;
      taskLog.push({ time: new Date().toLocaleTimeString(), text: "循环编排完成：" + (result.message || "") });
      if (taskLog.length > 8) taskLog = taskLog.slice(-8);
      messages[messages.length - 1] = { role: "assistant", content: result.message || "已完成。", details: detailLines };
    } else if (result.status === "clarify") {
      messages[messages.length - 1] = { role: "assistant", content: "❓ " + result.question + "\n（直接回复本消息即可继续当前任务）", details: detailLines };
    } else if (result.status === "failed") {
      activeTask = null;
      messages[messages.length - 1] = { role: "assistant", content: "❌ " + (result.message || "任务失败"), details: detailLines };
    } else {
      activeTask = null;
      messages[messages.length - 1] = {
        role: "assistant",
        content: "❌ Agent 决策协议无效，未执行任何画布动作。原因：" + (result.reason || "未知原因") + "。请重试；系统要求每一步单独返回 action / wait / finish，并先完成上游节点。",
        details: detailLines.concat([
          "循环决策解析失败（" + (result.reason || "未知原因") + "）",
          "已阻止旧版一次性 actions，避免产生无连线、无等待的孤立节点",
        ]),
      };
    }
    saveSession();
