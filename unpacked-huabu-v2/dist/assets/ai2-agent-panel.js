...
(function () {
  ...
  async function send() {
    ...
    try {
      if (window.__AI2_AGENT_LOOP && typeof window.__AI2_AGENT_LOOP.runAgentTask === "function") {
        ...
        await legacySend(b, plat, model, skill, text, null, null);
      }
    } catch (e) {
  ...
  /* 旧协议：一次性 actions（循环模块缺失时的兜底，也是循环失败时的自动回退路径） */
  ...
  /* 新协议：状态驱动循环编排（GLM 5.3 Flash 无限画布 Agent 编排规范）
     ...
     模型返回旧版 actions 数组或反复解析失败时，自动降级到 legacySend。 */
  async function loopSend(b, plat, model, skill, text) {
    ...
    if (result.status === "completed") {
      ...
      activeTask = null;
      await legacySend(b, plat, model, skill, text, result.rawContent || "", [
        "循环决策解析失败（" + (result.reason || "未知原因") + "），已自动降级为一次性执行",
      ]);
    }
    saveSession();
    renderMsgs();
