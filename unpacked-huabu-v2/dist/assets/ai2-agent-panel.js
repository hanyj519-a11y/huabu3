...
(function () {
  ...
  function buildMessages(skill) {
    var sys = [
      ...
      "参数规则：图片节点使用 model、ratio、quality；例如 1280x720 转为 ratio 16:9，2K/高清转为 quality 2k。视频节点使用 model、size、duration。保留用户明确说出的 prompt。",
  ...
  async function send() {
    ...
    try {
      ...
      var results = execActions(executableActions);
